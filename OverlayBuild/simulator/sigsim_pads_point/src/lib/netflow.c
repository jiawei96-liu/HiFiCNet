...
bool check_vlan_tag(struct netflow * nf)
{
    uint32_t ipv4_src = nf->match.ipv4_src;
    uint32_t ipv4_dst = nf->match.ipv4_dst;

    //assume: vlan netmask : 255.255.255.0   // node? check for future

    uint32_t valn_tag_src = ipv4_src >> 8;
    uint32_t valn_tag_dst = ipv4_dst >> 8;

    if(valn_tag_src == valn_tag_dst)
        return true;
...
netflow_update_send_time(struct netflow *flow, uint32_t port_speed)
{
  ...
  //printf("update send time, cnt: %d . port_speed %d\n",flow->byte_cnt,port_speed);
