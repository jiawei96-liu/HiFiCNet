...
def run_horse_expt(topo, k, flowsToCreate):
  ...
  for src_index, dest_index in flowsToCreate:
    ...
    dest_ip = topo.node_info[dst_name]["ip"]
    src.udp(dest_ip, args.start_time, duration = args.time, rate = 1) 
    # break

  end_time = args.time * 1000000 + args.start_time
  print end_time
  sim = Sim(topo, ctrl_interval = args.idle_ctrl, end_time = end_time, log_level = LogLevels.LOG_INFO)
  sim.start()
